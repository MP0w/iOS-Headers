//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class PUStackedAlbumLayout, PUStackedAlbumTransition;

@protocol PUStackedAlbumTransitionDelegate <NSObject>
- (PUStackedAlbumLayout *)stackedAlbumTransition:(PUStackedAlbumTransition *)arg1 layoutForCollection:(id <PLAssetContainer>)arg2;
- (void)stackedAlbumTransition:(PUStackedAlbumTransition *)arg1 setVisibility:(BOOL)arg2 forCollection:(id <PLAssetContainer>)arg3;
@end

